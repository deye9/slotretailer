...
func GetDML(id int) (report Reports, err error) {
	var rows *sql.Rows
	if rows, err = Get(fmt.Sprintf(`select id, title, query, (select concat(firstname, ' ', lastname) from users where reports.created_by = id ) as created_by, created_at, updated_at, deleted_at from reports where id = %d and deleted_at is null;`, id)); err != nil {
...
func GetReports() (reports []Reports, err error) {
	var rows *sql.Rows
	if rows, err = Get(`SELECT id, title, query, (select concat(firstname, ' ', lastname) from users where reports.created_by = id ) as created_by, created_at, updated_at, deleted_at FROM reports where deleted_at is null order by created_at desc;`); err != nil {
...
func UpdateReport(title, query string, recordID int) (err error) {
	updateCommand := fmt.Sprintf(`UPDATE reports SET title = "%s", query = "%s", updated_at = "CURRENT_TIMESTAMP" WHERE id = "%d";`, title, query, recordID)
